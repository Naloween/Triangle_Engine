...
class Player{
    ...
    move(dt){
        ...
        let vitesse = [this.vitesse[0], this.vitesse[1], this.vitesse[2]];
...
class Chunk{
    ...
    constructor(vertex_offset, position, size, side_length){
...
class Game{
    constructor(view, player, space_matter){

        this.player = player;
        this.view = view;
        this.space_matter = space_matter;
        ...
        this.camera = new te.Camera(width, height, render_distance);

        //Graphic engine
        this.engine = new te.TriangleEngine(view.getContext("webgl2"));
    ...
    update_world(){
        ...
        for (let chunk of this.chunks){
            let u = vec3.create();
            let position = [chunk.position[0] + chunk.size/2, chunk.position[1] + chunk.size/2, chunk.position[2] + chunk.size/2];
